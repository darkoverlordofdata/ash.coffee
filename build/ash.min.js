...
this.cacheTail;this.cacheTail=node.previous;this.dispose(node)}};return NodePool}();"use strict";ash.core.System=function(){function System(){}System.prototype.previous=null;System.prototype.next=null;System.prototype.priority=0;System.prototype.addToEngine=function(engine){};System.prototype.removeFromEngine=function(engine){};System.prototype.update=function(time){};return System}();"use strict";ash.core.SystemList=function(){function SystemList(){}SystemList.prototype.head=null;SystemList.prototype.tail=
null;SystemList.prototype.add=function(system){var node;if(!this.head){this.head=this.tail=system;system.next=system.previous=null}else{node=this.tail;while(node){if(node.priority<=system.priority)break;node=node.previous}if(node===this.tail){this.tail.next=system;system.previous=this.tail;system.next=null;this.tail=system}else if(!node){system.next=this.head;system.previous=null;this.head.previous=system;this.head=system}else{system.next=node.next;system.previous=node;node.next.previous=system;node.next=
system}}};SystemList.prototype.remove=function(system){if(this.head===system)this.head=this.head.next;if(this.tail===system)this.tail=this.tail.previous;if(system.previous)system.previous.next=system.next;if(system.next)system.next.previous=system.previous};SystemList.prototype.removeAll=function(){var system;while(this.head){system=this.head;this.head=this.head.next;system.previous=null;system.next=null}this.tail=null};SystemList.prototype.get=function(type){var system;system=this.head;while(system){if(system.constructor===
type)return system;system=system.next}return null};return SystemList}();"use strict";ash.core.Family=function(){function Family(){}Family.prototype.nodes=null;Family.prototype.newEntity=function(entity){throw new Error("Method must be overriden");};Family.prototype.removeEntity=function(entity){throw new Error("Method must be overriden");};Family.prototype.componentAddedToEntity=function(entity,componentClass){throw new Error("Method must be overriden");};Family.prototype.componentRemovedFromEntity=
function(entity,componentClass){throw new Error("Method must be overriden");};Family.prototype.cleanUp=function(){throw new Error("Method must be overriden");};return Family}();"use strict";EntityList=ash.core.EntityList;Dictionary=ash.ext.Dictionary;NodeList=ash.core.NodeList;NodePool=ash.core.NodePool;Util=ash.ext.Util;ash.core.ComponentMatchingFamily=function(){ComponentMatchingFamily.prototype.nodes=null;ComponentMatchingFamily.prototype.entities=null;ComponentMatchingFamily.prototype.nodeClass=
null;ComponentMatchingFamily.prototype.components=null;ComponentMatchingFamily.prototype.nodePool=null;ComponentMatchingFamily.prototype.engine=null;function ComponentMatchingFamily(_at_nodeClass,_at_engine){this.nodeClass=_at_nodeClass;this.engine=_at_engine;this.releaseNodePoolCache=__bind(this.releaseNodePoolCache,this);this.init()}ComponentMatchingFamily.prototype.init=function(){var name,type,_ref;this.nodes=new NodeList;this.entities=new Dictionary;this.components=new Dictionary;this.nodePool=
new NodePool(this.nodeClass,this.nodeClass.components);_ref=this.nodeClass.components;for(name in _ref){type=_ref[name];this.components[Util.getClassName(type)]=type}};ComponentMatchingFamily.prototype.newEntity=function(entity){this.addIfMatch(entity)};ComponentMatchingFamily.prototype.componentAddedToEntity=function(entity,componentClass){this.addIfMatch(entity)};ComponentMatchingFamily.prototype.componentRemovedFromEntity=function(entity,componentClass){var name;name=Util.getClassName(componentClass)!=
...
function(nodeClass){if(Util.getClassName(nodeClass)in this.families){this.families[Util.getClassName(nodeClass)].cleanUp();delete this.families[Util.getClassName(nodeClass)]}};Engine.prototype.addSystem=function(system,priority){system.priority=priority;system.addToEngine(this);this.systemList.add(system)};Engine.prototype.getSystem=function(type){return this.systemList.get(type)};Engine.prototype.removeSystem=function(system){this.systemList.remove(system);system.removeFromEngine(this)};Engine.prototype.removeAllSystems=
...
nodes){var klass,name,property,type,_ref;this.components={};this.nodes={};if(components!=null)for(name in components){klass=components[name];this.components[name]=klass}if(nodes!=null)for(name in nodes){klass=nodes[name];if(klass.components==null){klass.components={};_ref=klass.prototype;for(property in _ref){type=_ref[property];klass.components[property]=type;klass.prototype[property]=null}klass.prototype.entity=null;klass.prototype.previous=null;klass.prototype.next=null}if(components!=null)this.nodes[name]=
klass}}return Helper}();ash.ext.Util=function(){function Util(){}Util.getClassName=function(klass){var _ref;return(_ref=klass.className)!=null?_ref:klass.name};return Util}();"use strict";ash.fsm.ComponentInstanceProvider=function(){ComponentInstanceProvider.prototype.instance=null;function ComponentInstanceProvider(_at_instance){this.instance=_at_instance}ComponentInstanceProvider.prototype.getComponent=function(){return this.instance};ComponentInstanceProvider.prototype.getIdentifier=function(){return this.instance};
return ComponentInstanceProvider}();"use strict";ash.fsm.ComponentSingletonProvider=function(){ComponentSingletonProvider.prototype.componentType=null;ComponentSingletonProvider.prototype.instance=null;function ComponentSingletonProvider(type){this.componentType=type}ComponentSingletonProvider.prototype.getComponent=function(){if(this.instance==null)this.instance=new this.componentType;return this.instance};ComponentSingletonProvider.prototype.getIdentifier=function(){return this.getComponent()};
...
FrameTickProvider.prototype.start=function(){this.request=requestAnimationFrame(this.dispatchTick);this.isPlaying=true};FrameTickProvider.prototype.stop=function(){cancelAnimationFrame(this.request);this.isPlaying=false};FrameTickProvider.prototype.dispatchTick=function(timestamp){var frameTime,temp;if(!this.isPlaying)return;if(this.showStats)this.begin();temp=this.previousTime||timestamp;this.previousTime=timestamp;frameTime=(timestamp-temp)*.001;this.dispatch(frameTime);this.request=requestAnimationFrame(this.dispatchTick);
